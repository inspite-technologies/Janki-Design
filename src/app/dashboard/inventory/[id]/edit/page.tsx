...
export default function EditMaterialPage() {
    ...
    useEffect(() => {
        const fetchMaterial = async () => {
            try {
                ...
                // Handle unwrapping if necessary, similar to other endpoints
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setSubmitting(true);
        try {
            await InventoryAPI.updateMaterial(id, {
                ...formData,
    ...
    return (
        <div className="flex flex-col h-full bg-background-light dark:bg-background-dark">
            <header className="bg-surface-light dark:bg-surface-dark border-b border-border-light dark:border-border-dark px-6 py-4 flex items-center gap-4 shrink-0">
                ...
                <div>
                    <h1 className="text-xl font-bold text-text-primary dark:text-text-primary-dark">Edit Material</h1>
                    <p className="text-xs text-text-secondary dark:text-text-secondary-dark">Update inventory item</p>
                </div>
            </header>
